...
import { OneCouponThunk } from "../../RTK/Thunk/OneCouponThunk";
import { UpdateCouponThunk } from "../../RTK/Thunk/UpdateCouponThunk";
...
const CouponEdit = () => {
    ...
    // ========get data=========
    const getCouponDataRef = useRef(true);
    useEffect(() => {
        if (getCouponDataRef.current && parm.editCoupon) {
            dispatch(OneCouponThunk({ id: parm.editCoupon }));
            getCouponDataRef.current = false;
        }
    }, [dispatch, parm.editCoupon]);
    ...
    useEffect(() => {
        if (oneCoupon.name) {
            setInputValue({
                name: oneCoupon.name,
                percentage: oneCoupon.percentage,
                maxUsers: oneCoupon.remaining_users_count,
            });
            setStartDate(new Date(oneCoupon.start_date));
            setEndDate(new Date(oneCoupon.end_date));
        }
    }, [oneCoupon]);
    ...
    let handleSubmit = (e) => {
        ...
        // console.log(data);
        dispatch(UpdateCouponThunk(data))
            .unwrap()
            .then((data) => {
                // console.log(data);
...
export default React.memo(CouponEdit);
